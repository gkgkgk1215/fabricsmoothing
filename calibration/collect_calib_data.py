"""
Use this code to have the robot automatically collect data for calibration.  It
should be run in a two-stage process. The first time should use manual
intervention to determine workspace and height boundaries.  The second time is
when the real action begins and the trajectories can be run a lot.

TODO: code saves as we go, not at the end, so that it's robust to cases when
the dVRK might fail (e.g. that MTM reading error we've been seeing a lot
lately).

When running this for real, be sure to feed (i.e. `tee`) the output into a file
for my own future reference. Again, as I said earlier, we're in two cases,
depending on whether a `boundaries.p` file already exists.

For the second case, there's a bunch of arguments, but no argparse since this
isn't being run many times in simulation.

Note: `z_offset` is for generic offsets to avoid damaging the surface, but
`z_offset_home` is only for the four `home` positions.
...
# TODO: direct copy from ICRA 2018 code, need to revise for any changes.
...
if __name__ == "__main__":
    ...
    data_path = 'calibration/boundaries.p'

    # Set up the dvrk, move to an origin, close gripper.
    p = dvrkClothSim()
    ...
    if not args.collect_data:
        print('NOT collecting data --- just doing manual checks of boundaries.')
        assert not os.path.exists(data_path), '{} exists!'.format(data_path)
        # Move to home position?
        #utils.move(arm=arm, pos=[0.0,0.06,-0.13], rot=[0,-10,-170], SPEED_CLASS='Slow')
        collect_boundaries(p, cam, data_path)
    else:
        assert os.path.exists(data_path), '{} does not exist!'.format(data_path)
        print('Now collecting data!')
        # TODO: depends on what args will be user.
        #args = {}
        #args['d'] = d
        #args['arm'] = arm
